...
import { MutableMap, createMutableMap } from './map';
...
const transcripts: MutableMap<string> = createMutableMap<string>();
...
export const save = (req: SafeRequest, res: SafeResponse): void => {
  ...
  const replaced = transcripts.containsKey(name)
...
export const load = (req: SafeRequest, res: SafeResponse): void => {
  ...
  if (!transcripts.containsKey(name)) {
    res.status(404).send(`transcript with name "${name}" not found`);
    return;
  }
  const transcript = transcripts.getVal(name);
