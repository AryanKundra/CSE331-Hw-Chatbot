...
describe('routes', function() {
  ...
  load(req1, res1);
  assert.deepStrictEqual(res1._getStatusCode(), 400);
  assert.deepStrictEqual(res1._getData(), 'required query parameter "name" was missing');

  // Second branch, straight line code, error case
  const req2 = httpMocks.createRequest({method: 'GET', url: '/load', query: { name: 'nonexistent' }});
  const res2 = httpMocks.createResponse();
  load(req2, res2);
  assert.deepStrictEqual(res2._getStatusCode(), 404);
  assert.deepStrictEqual(res2._getData(), 'transcript with name "nonexistent" not found');
